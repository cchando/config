==== Adding a package to Nix Packages collection
To do this, we generally need to do 3 things:
...
Attributes can be selected from a set using the `.` operator. For instance,
    `{ a = "Foo"; b = "Bar"; }.a`
evaluates to "Foo". It is possible to provide a default value in an attribute selection using the `or` function. For example,
